 ...
 * ECE220 Spring 2018
 *
 * Program name: print_reverse.c, a recursive backwards string printer
 *
...
main ()
{
    char     buf[100];	// buffer to hold string entered
    int32_t  len;	// length of string

    // Prompt the user.
    printf ("Enter a string: ");

    // Read a string.  On failure, give up.
    if (1 != scanf ("%99s", buf)) {
        printf ("That's a strange string!\n");
	return 3;
    }

    // Print reversed string and length
    printf ("Your string reversed is \"");
    len = print_reverse (buf);
    printf ("\".\nThe length is %d characters.\n", len);
